...
router.post('/simpan', upload.single('logo'), (req,res) => {
  ...
  run(`UPDATE identitas_sekolah SET nama_sekolah=?,alamat=?,tahun_ajaran=?,telp=?,email=?,website=?,logo=?,updated_at=datetime('now','localtime') WHERE id=1`,
      [nama_sekolah,alamat,tahun_ajaran,telp||'',email||'',website||'',logo]);
